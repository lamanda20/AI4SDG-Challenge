...
from schemas import RegisterRequest, LoginRequest, TokenResponse, UserFormInput
from crud import get_user_by_email, create_client, save_plan
from services.feature_engineering import compute_features
from agents.motivator_ml import run_ml_analysis
from services.vector_query import build_rag_query
from agents.clinician_rag import retrieve_guidelines
from agents.prescriber_llm import generate_plan
...
def _resolve_login_user(db: Session, identifier: str):
    alias = identifier.strip().lower()
    admin_alias = os.getenv("ADMIN_LOGIN_ALIAS", "---admin---").strip().lower()

    if alias == admin_alias:
        preferred_admin_email = os.getenv("ADMIN_LOGIN_EMAIL", "").strip().lower()
        if preferred_admin_email:
            return get_user_by_email(db, preferred_admin_email)

        return db.query(Admin).order_by(Admin.created_at.desc()).first()

    return get_user_by_email(db, identifier.strip())
...
def register(data: RegisterRequest, db: Session = Depends(get_db)):
    ...
    return TokenResponse(access_token=_create_token(client.id), role="client")
...
def login(data: LoginRequest, db: Session = Depends(get_db)):
    ...
    return TokenResponse(access_token=_create_token(user.id), role=user.role)
